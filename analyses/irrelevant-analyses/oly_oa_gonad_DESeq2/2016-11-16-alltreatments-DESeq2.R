...
data <- read.table("/Users/yaamini/Documents/yaaminiv-fish546-2016/data/2016-11-16_count_data/2016-11-16-oly-gonad-oa-count-data.txt", header = T, sep = "\t")
...
png("/Users/yaamini/Documents/yaaminiv-fish546-2016/analyses/irrelevant-analyses/oly_oa_gonad_DESeq2/alltreatments.png", width = 800, height = 800)
plot(tmp$baseMean, tmp$log2FoldChange, pch=20, cex=0.45, ylim=c(-3, 3), log="x", col="darkgray", main="Differentially Expressed Genes in 106 vs. 108  (pval <= 0.05)", xlab="mean of normalized counts", ylab="Log2 Fold Change")
#Getting the significant points and plotting them again so they're a different color
tmp.sig <- deseq2.res[!is.na(deseq2.res$padj) & deseq2.res$padj <= 0.05, ]
points(tmp.sig$baseMean, tmp.sig$log2FoldChange, pch=20, cex=0.45, col="violetred")
...
write.table(tmp.sig, "/Users/yaamini/Documents/yaaminiv-fish546-2016/analyses/irrelevant-analyses/oly_oa_gonad_DESeq2/alltreatments_DEG.tab", row.names = T)
...
#Step 7: Save plot as a new png file
png("/Users/yaamini/Documents/yaaminiv-fish546-2016/analyses/irrelevant-analyses/oly_oa_gonad_DESeq2/alltreatments-p0.5-.png", width = 800, height = 800)
plot(tmp$baseMean, tmp$log2FoldChange, pch=20, cex=0.45, ylim=c(-3, 3), log="x", col="darkgray", main="Differentially Expressed Genes in 106 vs. 108  (pval ≤ 0.5)", xlab="mean of normalized counts", ylab="Log2 Fold Change")
#Getting the significant points and plotting them again so they're a different color
tmp.sig <- deseq2.res[!is.na(deseq2.res$padj) & deseq2.res$padj <= 0.5, ]
points(tmp.sig$baseMean, tmp.sig$log2FoldChange, pch=20, cex=0.45, col="violetred")
...
#Step 8: Create table with differentially expressed contigs
write.table(tmp.sig, "/Users/yaamini/Documents/yaaminiv-fish546-2016/analyses/irrelevant-analyses/oly_oa_gonad_DESeq2/alltreatments-p0.5-_DEG.tab", row.names = T)